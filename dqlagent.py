...
class DQLAgent:
    ...
    def act(self, state):
        ...
        # NEW SPEED OPTIMIZATION: call the model directly to avoid the overhead of model.predict() for a single row.
        return np.argmax(self.model(state, training=False).numpy()[0])
        # return np.argmax(self.model.predict(state)[0])
        
    def replay(self):
        batch = random.sample(self.memory, self.batch_size)
        # NEW SPEED OPTIMIZATION: batch all predictions and train once instead of predicting/fitting once per sample.
        ...
        targets[np.arange(len(batch)), actions] = (
            rewards + self.gamma * np.max(next_values, axis=1) * (~dones)
        )
    ...
    def test(self, episodes, min_accuracy=0.0,
        ...
        for e in range(1, episodes + 1):
            ...
            for f in range(1, 5001):
                # NEW SPEED OPTIMIZATION: call the model directly to avoid model.predict() overhead in the test loop.
                action = np.argmax(self.model(state, training=False).numpy()[0])
